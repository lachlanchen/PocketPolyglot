...
usage() {
  cat <<'USAGE'
Usage: scripts/interlinear/start_trilingual_grammar_tmux.sh <book-id> [session]

Start a resumable tmux grammar-color role pass for a trilingual EN/JP/ZH book.
The pass watches books/<book-id>/book-plan.json chunk outputs, adds English role
labels to token field "g", and periodically compiles the 12 standard PDFs.
...
book_id="$1"
session="${2:-zhjpbook-${book_id}-trilingual-grammar}"
