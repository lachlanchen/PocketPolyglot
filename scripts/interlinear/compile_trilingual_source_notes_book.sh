...
usage() {
  cat <<'USAGE'
Usage: scripts/interlinear/compile_trilingual_source_notes_book.sh --book-id <id> [--color-mode color|blackwhite]

Compile a trilingual source-main edition for Japanese classical/kana books:
original Japanese source as main text with indented English and Chinese notes.
USAGE
}
...
title_ja="$(jq -r '.book_title_ja // .book_title_en' "$plan")"
...
build_dir="build/$book_id/wayakana-main-en-zh/$color_mode"
assembled_json="books/$book_id/work/trilingual/preview/$book_id.source-main.json"
base_title="${title_ja}（英文・中文注）"
...
python scripts/interlinear/json_to_trilingual_source_notes_tex.py "$assembled_json" \
...
xelatex -interaction=nonstopmode -halt-on-error -jobname=book -output-directory="$build_dir" \
  "\\def\\TriSourceNotesSource{$build_dir/source.tex}\\input{tex/interlinear-trilingual-source-notes/book.tex}" \
