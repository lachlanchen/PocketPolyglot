...
COVER_ROOT = ROOT / "assets" / "covers" / "sanxingdui"
...
def cover_for_slug(slug: str) -> Path | None:
    cover = COVER_ROOT / slug / "cover.png"
...
def write_source(
    ...
    *,
    ...
    subtitle = "口袋文本版" if pocket else "文本版"
    image_note = "；并嵌入原书页图像以保留图版、地图与原始图注" if source_pdf and image_dir else ""
    note = f"AgInTiFlow curated · https://flow.lazying.art · powered by LazyingArt{image_note}"
    ...
    with output.open("w", encoding="utf-8") as handle:
        ...
        if cover_image:
            rel_cover = cover_image.relative_to(ROOT).as_posix()
            handle.write(f"\\SXCoverPage{{{rel_cover}}}\n")
        handle.write(
            "\\SXTitle"
            f"{{{tex_escape(title)}}}"
...
def write_booklike_source(
    ...
    *,
    ...
    output.parent.mkdir(parents=True, exist_ok=True)
    rel_md = book.markdown.relative_to(ROOT)
    subtitle = "图文口袋版" if pocket else "图文书版"
    note = "AgInTiFlow curated · https://flow.lazying.art · powered by LazyingArt"
...
def build_one(book_info: dict[str, str], args: argparse.Namespace) -> Path:
    ...
    writer(
        ...
        image_dir=image_dir,
        image_dpi=image_dpi,
        cover_image=cover_for_slug(slug),
    )
    if args.no_compile:
        return source_tex
