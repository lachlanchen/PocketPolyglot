# Language Set Exports

Generated at: `2026-06-13T16:18:42.205674+00:00`
Nutstore root: `/home/lachlan/Nutstore Files/Projects/LinguaLeaf`

| Set | Folder | Books | PDFs | Color | Black-white | Notes |
| --- | --- | ---: | ---: | ---: | ---: | --- |
| EN/JP/ZH | `en-main-jp-zh` | 19 | 38 | 19 | 19 | English main with indented Japanese and Chinese notes. |
| JP/ZH trilingual leftovers | `jp-zh-trilingual-leftovers` | 11 | 44 | 22 | 22 | Ordinary Japanese/Chinese paired editions that do not yet have English added. |
| Wenyanwen/JP/ZH trilingual leftovers | `wenyanwen-jp-zh-trilingual-leftovers` | 3 | 12 | 6 | 6 | Classical Chinese-oriented editions that do not yet have English added; zh means modern Chinese helper/annotation. |

## Detail

### EN/JP/ZH

Folder: `en-main-jp-zh`

See that folder's `manifest.json` for the full item list.

### JP/ZH trilingual leftovers

Folder: `jp-zh-trilingual-leftovers`

| Book | Title | PDFs |
| --- | --- | ---: |
| `chumon-no-ooi-ryoriten` | 注文の多い料理店 / 要求太多的餐馆 | 4 |
| `genji-modern` | 源氏物語 / 源氏物语 | 4 |
| `ginga-tetsudo` | 銀河鉄道の夜 / 银河铁道之夜 | 4 |
| `izu-no-odori` | 伊豆の踊子 / 伊豆的舞女 | 4 |
| `kinkakuji` | 金閣寺 / 金阁寺 | 4 |
| `kokoro` | こころ / 心 | 4 |
| `no-longer-human` | 人間失格 / 人间失格 | 4 |
| `rashomon-stories` | 羅生門短篇集 / 罗生门短篇集 | 4 |
| `sichuan-folk-stories-vol1` | 中国民间故事集成 四川卷 上 | 4 |
| `snow-country` | 雪国 | 4 |
| `the-old-capital` | 古都 | 4 |

### Wenyanwen/JP/ZH trilingual leftovers

Folder: `wenyanwen-jp-zh-trilingual-leftovers`

| Book | Title | PDFs |
| --- | --- | ---: |
| `shiji-aginti` | 史記 | 4 |
| `sishu-jizhu` | 四書章句集註 | 4 |
| `sishu-jizhu-aginti` | 四書章句集注 AgInTi edition | 4 |
