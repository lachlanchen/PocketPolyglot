...
For classical Chinese / wenyan projects, default to the maximum-language quadrilingual shape: wenyan main text, English, readable modern Japanese, and modern Chinese. Generate with strict chunk validation, normalized grammar roles, and color-capable JSON. The preferred large-run setting is `WORKERS=100 MODEL=gpt-5.5 REASONING=low` when the manifest is already prepared and validation is cheap to rerun. Final PDFs should use the larger PocketPolyglot font profile (`大字版` / large-font naming), with both color and black-white variants.

For technical textbooks and music-technique books, preserve figures, equations, notation, tables, diagrams, captions, and exercise layouts as first-class content. Do not flatten formulas, chord diagrams, fretboards, graphs, or tables into prose. The TeX output should contain real text/math where possible and referenced figure assets where needed, with source-page evidence and overfull-line checks before calling the book complete. Never treat page-image-only or facsimile-style output as a completed technical pocket book; if the local PDF-to-TeX pipeline cannot produce real TeX, mark the task blocked with evidence.
